...
@router.get("/applications/optimized",
            summary="Récupérer les candidatures avec données complètes")
async def get_applications_optimized(
    ...
    candidate_id: Optional[str] = Query(
        None, description="Filtrer par candidat"
    ),
    include_documents: bool = Query(True, description="Inclure les documents"),
    include_evaluations: bool = Query(True, description="Inclure les évals"),
...
@router.get("/dashboard/stats/optimized",
            summary="Statistiques dashboard optimisées")
async def get_dashboard_stats_optimized(
    recruiter_id: Optional[str] = Query(
        None, description="ID du recruteur (optionnel)"
    ),
    db: Any = Depends(get_db),
    current_user: Any = Depends(get_current_user)
):
    ...
    try:
        ...
        if not recruiter_id and current_user.get("role") == 'recruiter':
...
@router.get("/candidates/{candidate_id}/applications/optimized",
            summary="Candidatures d'un candidat optimisées")
async def get_candidate_applications_optimized(
    ...
    try:
        ...
        if user_role == 'candidate' and user_id != candidate_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Accès refusé. Candidatures personnelles uniquement"
            )

        if user_role not in ['candidate', 'recruiter', 'admin', 'observer']:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Accès refusé. Rôle non autorisé"
            )
...
async def get_performance_comparison(
    ...
    try:
        ...
        service = OptimizedQueryService(db)
        apps, total = await service.get_applications_with_full_data(limit=50)
        ...
        return {
            "comparison": {
                ...
                "improvement_percentage": round(
                    ((old_approach_time - new_approach_time) /
                     old_approach_time) * 100, 2),
                "applications_retrieved": len(apps),
