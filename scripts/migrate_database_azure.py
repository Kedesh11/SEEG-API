"""
Script de Migration SQL Direct sur Azure PostgreSQL - SEEG API
==============================================================

Ce script gère l'exécution robuste et sécurisée de migrations SQL
directement sur Azure PostgreSQL avec traçabilité complète.
...
✅ Connexion sécurisée à Azure PostgreSQL
...
Usage:
    python migrate_database_azure.py --action upgrade
    python migrate_database_azure.py --action status
    python migrate_database_azure.py --action rollback --target <version>
...
class DatabaseConfig:
    """Configuration de la base de données Azure PostgreSQL"""
    # ========================================
    ...
    user: str = "Sevan"
    password: str = "Sevan@Seeg"  # ⚠️ Modifier avec votre mot de passe
...
# ============================================================================
# GESTIONNAIRE DE FIREWALL AZURE
# ============================================================================

class AzureFirewallManager:
    """Gère automatiquement les règles de firewall Azure PostgreSQL"""
    
    def __init__(self, config: DatabaseConfig, logger: StructuredLogger):
        self.config = config
        self.logger = logger
        self.firewall_rule_name: Optional[str] = None
    
    async def ensure_firewall_access(self) -> bool:
        """
        S'assure que l'IP actuelle a accès au serveur PostgreSQL
        Crée une règle de firewall si nécessaire
        """
        import subprocess
        
        self.logger.info("Vérification de l'accès firewall...", ip=self.config.my_ip)
        
        # Nom de la règle basé sur l'IP
        self.firewall_rule_name = f"migration-{self.config.my_ip.replace('.', '-')}"
        ...
        try:
            ...
            check_cmd = [
                ...
                "--rule-name", self.firewall_rule_name
            ]
            ...
            if result.returncode == 0:
                self.logger.success(f"Règle firewall existe: {self.firewall_rule_name}")
                return True
            
            # La règle n'existe pas, la créer
            self.logger.info(f"Création de la règle firewall: {self.firewall_rule_name}")
            ...
            create_cmd = [
                ...
                "--name", self.config.server_name,
                "--rule-name", self.firewall_rule_name,
                "--start-ip-address", self.config.my_ip,
                "--end-ip-address", self.config.my_ip
            ]
            ...
            if result.returncode == 0:
                self.logger.success(f"Règle firewall créée: {self.firewall_rule_name}")
                return True
            else:
                self.logger.warning(f"Impossible de créer la règle firewall: {result.stderr}")
                self.logger.info("Continuons quand même, l'accès existe peut-être déjà...")
                return True
                
        except subprocess.TimeoutExpired:
            self.logger.warning("Timeout lors de la vérification du firewall")
            return True
        except Exception as e:
            self.logger.warning(f"Erreur firewall (non bloquante): {e}")
            return True
...
class DatabaseConnection:
    ...
    async def connect(self) -> bool:
        ...
        try:
            self.logger.info("Connexion à Azure PostgreSQL...",
...
class MigrationManager:
    ...
    def __init__(
        ...
        self.firewall = AzureFirewallManager(db_config, self.logger)
    ...
    async def initialize(self) -> bool:
        ...
        # 1. Vérifier/créer la règle de firewall
        await self.firewall.ensure_firewall_access()
        
        # 2. Se connecter à la base de données
        if not await self.db.connect():
            return False
        
        # 3. Initialiser les tables système
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Script de migration SQL direct pour Azure PostgreSQL avec auto-découverte",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemples:
  python migrate_database_azure.py --action upgrade
  python migrate_database_azure.py --action upgrade --force-modified
  python migrate_database_azure.py --action status
  python migrate_database_azure.py --action rollback --target 20251016_create_email_logs
  python migrate_database_azure.py --migrations-dir ./custom_migrations
        """
    )
    ...
    print()
    print("=" * 80)
    print("  MIGRATION SQL AZURE POSTGRESQL - SEEG API v2.0")
